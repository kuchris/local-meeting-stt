...
contextBridge.exposeInMainWorld("meetingApi", {
  runCommand: (kind: string, args?: Record<string, unknown>) =>
    ipcRenderer.invoke("run-command", kind, args ?? {}),
  stopCommand: (processId: number) =>
    ipcRenderer.invoke("stop-command", processId),
  ...
  startAssetDownload: (assetId: string) =>
    ipcRenderer.invoke("start-asset-download", assetId),
  stopAssetDownload: (assetId: string) =>
    ipcRenderer.invoke("stop-asset-download", assetId),
  listOutputSessions: (outputDir: string) =>
    ipcRenderer.invoke("list-output-sessions", outputDir),
  ...
  onProcessEvent: (callback: (event: ProcessEvent) => void) => {
    const listener = (_: Electron.IpcRendererEvent, event: ProcessEvent) =>
      callback(event);
