...
(async()=>{
  ...
  try {
    ...
    assert.equal(assets.find(asset=>asset.id==='whisper-cpp-turbo-model')?.downloadable,true);
    await page.getByLabel('Recognition model',{exact:true}).selectOption('turbo');
    assert.equal(await page.getByRole('button',{name:'Start meeting',exact:true}).isEnabled(),false);
